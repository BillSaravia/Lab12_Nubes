...
app.get('/', (req, res) => {
  connection.query('SELECT * FROM lab12', (error, results, fields) => {
...
app.get('/delete/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT foto FROM lab12 WHERE id = ?', [id]);
    ...
    await connection.promise().query('DELETE FROM lab12 WHERE id = ?', [id]);
    res.redirect('/');
  } catch (error) {
...
app.post('/save', upload.single('foto'), async (req, res) => {
  ...
  try {
    ...
    await connection.promise().query(
      'INSERT INTO lab12 SET ?',
      { nombre, apellidos, correo, fecha_nac, foto: s3Key }
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/edit/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT * FROM lab12 WHERE id = ?', [id]);
    const lab12 = row[0];
...
app.post('/update', upload.single('foto'), async (req, res) => {
  ...
  try {
    await connection.promise().query(
      'UPDATE lab12 SET nombre = ?, apellidos = ?, correo = ?, fecha_nac = ?, foto = ? WHERE id = ?',
      [nombre, apellidos, correo, fecha_nac, foto, id]
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/delete-all', async (req, res) => {
  try {
    const [rows] = await connection.promise().query('SELECT foto FROM lab12');
    ...
    await connection.promise().query('DELETE FROM lab12');
